...
NSString * time = [command.arguments objectAtIndex:0];
    ...
    ALAssetsGroupEnumerationResultsBlock assetsEnumerationBlock = ^(ALAsset *result, NSUInteger index, BOOL *stop) {
        if (result) {
            ...
            if ([self.assets count] == self.assetsCount)
            {
                ...
                [self getAllPhotoDateComplete:command with:nil];
            }
        }
    };
    ...
    ALAssetsLibraryAccessFailureBlock failureBlock = ^(NSError *error) {
        ...
        [self getAllPhotoDateComplete:command with:errorMessage];
    };
    ...
    NSUInteger groupTypes = ALAssetsGroupSavedPhotos; // ALAssetsGroupAlbum | ALAssetsGroupEvent | ALAssetsGroupFaces | ALAssetsGroupSavedPhotos | ALAssetsGroupPhotoStream;
